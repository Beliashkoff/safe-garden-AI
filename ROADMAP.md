...
### 3.3 Mobile ✅
- [x] Кнопка-скрепка с bottom-sheet «Камера» / «Галерея» в `chat_screen.dart`.
- [x] `permission_handler` за портом `PermissionPort` (`media_ports.dart`): камера всегда, фото — на iOS (на Android галерея через системный Photo Picker без разрешения); при отказе — диалог «Открыть настройки» (`openAppSettings`). Native: `NSCameraUsageDescription`/`NSPhotoLibraryUsageDescription`, Android `CAMERA`.
- [x] `image_picker` + `flutter_image_compress` (1920×1080, q=85, всегда JPEG) за портами `ImagePickerPort`/`ImageCompressorPort`.
- [x] Прогресс загрузки presign → PUT (прямой `Dio`, `onSendProgress`) → POST /messages в `message_composer.dart` (`UploadApi`); прогресс на превью в стрипе. При сбое загрузки сообщение не отправляется, стейджинг сохраняется.
- [x] Превью фото в bubble: блоки `image` рендерятся через `MediaCache` (`storage_key` → локальный файл; для истории — ленивая докачка через новый `POST /v1/uploads/view`). `ContentBlock.storage_key`, `message_photos.dart`.
- [x] До 4 фото за сообщение (лимит в `MessageComposer`); сообщение «только фото» без текста разрешено.

_Дополнительно к mobile-скоупу: на бэкенд добавлен `POST /v1/uploads/view` (presigned GET, ownership по префиксу `u/{user_id}/`, TTL 5 мин) — без него фото из истории (переустановка/другое устройство) нечем показать. `objstore.PresignGet`, `upload.PresignView`, OpenAPI обновлён._

### 3.4 Тесты
- [x] Backend: integration на ownership чужого ключа → 403 (`/uploads/view`), foreign image_ref → 404 (`/messages`) + presign-валидация.
- [x] Mobile: тест `MessageComposer` (pick→compress→upload→send, лимит 4, permission denied, сбой загрузки) с моками портов/`UploadApi`/`MediaCache`; контроллер — image-блоки и retry с фото.
...
### 4.3 Mobile
- [x] Кнопка-микрофон (hold-to-record как в Telegram; пустое поле → микрофон, есть текст → отправка).
- [x] `record` — запись в m4a/AAC (`AudioRecorderPort`, aacLc/mono/16kHz).
- [x] Индикатор уровня звука + таймер (`onAmplitudeChanged` → нормализованный 0..1, тикер 100мс, авто-стоп 60с).
- [x] Возможность отмены (свайп влево во время записи).
- [x] Прослушивание перед отправкой (превью с плеером `just_audio` + «Удалить»/«Отправить»).
- [x] Отображение в чате: плеер + текст транскрипции (`VoiceMessagePlayer`; SSE-событие `transcription` → блок к user-сообщению; оффлайн-кэш блоков переведён на JSON).

### 4.4 Тесты
- [x] Backend: мок транскрайбера, тест полного цикла (юнит `transcribeAudio` + integration voice happy-path/foreign/too-long — Этап 4.2).
- [x] Mobile: тест на разрешения микрофона (`voice_recorder_controller_test`: permission denied / record→preview / cancel / sendPreview).
...
- [x] Мини-админка для редактирования каталога удобрений. ✅ Реализована раньше графика (вынесена из бэклога): веб-панель `admin/` (React+MUI) + бэкенд `/admin/v1`, привязка к `ADMIN_EMAIL`, CRUD каталога питает `recommend_fertilizer`, дашборд/ошибки/журнал/сессии. Деплой — `admin.agronomai.site` через edge-Caddy. См. README «Админ-панель» и `ARCHITECTURE.md` §15.
