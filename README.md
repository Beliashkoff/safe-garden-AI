...
- **Авторизация:** Яндекс ID, VK ID, email-OTP — только российские способы входа (406-ФЗ)
...
- 🔐 **Миграция авторизации на РФ-провайдеры (406-ФЗ)** ✅ (2026-06-12) — Apple/Google полностью удалены; вход: **Яндекс ID** (authorization code + PKCE через системный браузер, обмен кода и проверка identity-JWT HS256 — на бэке) и **VK ID** (официальный `vkid_flutter_sdk`, confidential flow: SDK отдаёт только `code`+`device_id`, обмен с PKCE — на бэке) + email-OTP. Новое: эндпоинты `/v1/auth/{yandex,vk}/{start,complete}`, таблица `oauth_states` (одноразовый hashed state, ttl 10 мин, анти-флуд по IP), миграция `0013` (`yandex_sub`/`vk_sub` вместо `apple_sub`/`google_sub`), `internal/auth/{yandex,vk,pkce}.go`, конфиг `YANDEX_*`/`VK_*`. Тесты зелёные: backend `go test ./...` + integration (testcontainers, fake-провайдеры), mobile `flutter analyze` + 78 `flutter test`. _Осталось от заказчика: регистрация приложений в кабинетах Яндекс OAuth и VK ID + ключи (см. `.env.example`, `mobile/README.md`)._
- 🛠 **Админ-панель** ✅ (код) — веб-панель (`admin/`, React 19 + TypeScript + MUI v7 + Vite, фирменный зелёный `#2E7D32`) для оператора-неразработчика. Вкладки: **дашборд** (пользователи/вопросы/токены/расход/клики/ошибки + график за 30 дней), **каталог удобрений** (CRUD: то, что нативно рекомендует ИИ — название, «для чего», проблемы из закрытого enum инструмента `recommend_fertilizer`, растения, опц. цена/фото/ссылка; изменения сразу видны ИИ через тот же `fertilizers`), **ошибки** (лента 5xx без PII), **журнал действий**, **настройки** (смена пароля, активные сессии). Бэкенд `/admin/v1/*`: вход email+пароль, привязанный к `ADMIN_EMAIL` (только `agronomai@yandex.com`); первичная настройка и сброс пароля — по одноразовому коду на эту же почту (SMTP Yandex 360); opaque-сессии (sha256, sliding TTL, httpOnly+Secure+SameSite cookie); DB-brute-force-гард + метрика `admin_login_failures_total` + алерт «[АДМИН-ПАНЕЛЬ]». Миграция `0014` (`admin_users`/`admin_sessions`/`admin_codes`/`admin_audit_log`/`admin_error_events` + `fertilizers.price_rub`). Edge-Caddy: `admin.agronomai.site` → статика + `/admin/v1/*` в бэкенд. Тесты зелёные: backend `go test ./...`, `vite build`. _Деплой в прод (DNS `admin.*`, миграция 0014, `ADMIN_EMAIL` в `.env`, GitHub var `ADMIN_DOMAIN`) — runbook оператора._
...
- **Авторизация (сейчас):** регистрация приложения в Яндекс OAuth (`oauth.yandex.ru`) и кабинете VK ID (`id.vk.ru/about/business`, нужен VK Business ID + ИНН) — без ключей кнопки Яндекс/VK не работают.
- **Этап 7:** Apple Developer Account, Google Play Console / RuStore, Privacy Policy / ToS на двух языках.
