...
| ------------ | ------------ | ---------- |
| `ENV`        | `dev`        | `dev` / `prod`. Влияет на формат логов и обязательность ряда полей. Отдельного stage-окружения нет — см. SPEC §8 D15. |
...
2. Создать каталог (folder) `safe-garden-prod`. Отдельный stage-каталог не заводим — окружение одно (см. SPEC §8 D15).
