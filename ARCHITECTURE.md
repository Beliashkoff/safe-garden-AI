...
Веб-панель для оператора-неразработчика (владельца/контент-менеджера). Две задачи: **мониторинг приложения** и **управление каталогом удобрений**, которые ИИ нативно рекомендует пользователям. Реализована вне основного релизного графика мобильного приложения.

### 15.1 Топология

```
admin.agronomai.site ──TLS(LE)──> edge Caddy (та же API-VM)
                                    ├── /admin/v1/*  ── reverse_proxy ─> api:8080  (контейнер api, тот же бэкенд)
                                    └── /*           ── reverse_proxy ─> admin:80  (контейнер статики SPA)
```

- **SPA** (`admin/`): React 19 + TypeScript + MUI v7 + Vite, фирменный зелёный `#2E7D32` (как в мобильном приложении). Собирается в Docker (`admin/Dockerfile`, node-build → `caddy:2-alpine` отдаёт статику с SPA-fallback). Образ `ghcr.io/.../admin`, в матрице `build-images.yml`.
- **API** живёт в том же Go-бэкенде, что и `/v1` — отдельный sub-router `/admin/v1` (`internal/transport/http/adminapi`), смонтированный только когда задан `ADMIN_EMAIL`. Никаких новых процессов/портов.
- `ADMIN_DOMAIN` пуст → edge-Caddy обслуживает `admin.localhost` (внутренний серт, без ACME) — панель не выставлена наружу, пока оператор не направит DNS.

### 15.2 Аутентификация (один оператор)

- Единственный аккаунт привязан к `ADMIN_EMAIL` (прод: `agronomai@yandex.com`). Вход разрешён только под этим адресом; коды настройки/сброса уходят только на него.
- **Первичная настройка:** пока `admin_users` пуст — запрос кода (SMTP) → ввод кода + пароль → создаётся аккаунт + сессия.
- **Вход:** email + пароль (`bcrypt`, cost 12). Единый ответ при любой ошибке (нет enumeration). DB-brute-force-гард: ≥5 неудач с одного IP за 15 мин → отказ; метрика `admin_login_failures_total` → алерт `AdminLoginBruteForce`.
- **Сброс пароля:** код на `ADMIN_EMAIL` (тот же SMTP, что OTP пользователей) → новый пароль → ревок всех сессий.
- **Сессии:** opaque-токен (256 бит, в БД только sha256 — как refresh-токены), sliding TTL (по умолчанию 24ч), cookie `httpOnly`+`Secure`(prod)+`SameSite=Strict`, `Path=/admin`. Защита от cross-site мутаций — сверка `Origin` с `ADMIN_ALLOWED_ORIGIN`.

### 15.3 Каталог удобрений (ядро)

CRUD над таблицей `fertilizers` (см. §6). Каталог — единственный источник нативных рекомендаций: ИИ зовёт инструмент `recommend_fertilizer`, бэкенд отвечает товарами из этой таблицы (mTLS-callback, §11). Поэтому:

- **Пустой каталог → ИИ не рекомендует ничего** (только текстовый совет). Как только оператор добавил товар — он сразу доступен ИИ (общая таблица, без кэша/перевыката).
- `problems` ограничены закрытым enum инструмента (`llm.FertilizerProblemKeys` — единственный источник истины, общий для tool-схемы и валидации в админке). Это связь «диагноз ИИ → подбор товара».
- Нативность: ИИ отдаёт только **название** и **«для чего»** (`short_desc`). Цена (`price_rub`, новая колонка, миграция `0014`) упоминается ИИ **только если пользователь сам спросил** (правило в `system_v1.md`). Фото/ссылка — опционально, для карточки.
- Загрузка фото товара — серверный `PutPublic` в Object Storage (публичный URL для карточки; это НЕ путь пользовательских медиа, §8).

### 15.4 Мониторинг и журналирование

- **Дашборд:** активные пользователи, новые за 7д, вопросы к ИИ, токены/расход за 30д, клики по карточкам, размер каталога, ошибки за 24ч + график активности (агрегации в `admin_stats.sql`, read-only).
- **Лента ошибок** (`admin_error_events`): middleware `observability.ErrorEvents` пишет каждый 5xx (включая паники) с route-паттерном и `request_id`. **Без PII и тел запросов** (инвариант №3) — только цепочка `error`-строк. Чистится `cmd/cleanup` (30 дней).
- **Журнал действий** (`admin_audit_log`): входы, изменения каталога, смена пароля — `details` без PII.
- **Алерты** (Alertmanager): `component=admin` → отдельный получатель с темой `[АДМИН-ПАНЕЛЬ] …`, чтобы оператор отличал сбой/атаку именно в админке.

### 15.5 Схема БД (миграция 0014)

`admin_users` (один оператор, `bcrypt`), `admin_sessions` (sha256-токен, sliding TTL), `admin_codes` (одноразовые коды setup/reset, `bcrypt`, лимит попыток, TTL), `admin_audit_log`, `admin_error_events`, + колонка `fertilizers.price_rub`. Чистка просроченных сессий/кодов и старых ошибок — в `cmd/cleanup`.
